	...
	if (mcuCMDBuf) {
		...
		if (mcuCMDindex == mcuCMDBufSize) {
			...
			HAL_Write16(REG_CMD_WRITE,
					(FTU16)HAL_EVELoopMemWr(RAM_CMD,HAL_Read32(REG_CMD_WRITE),CMDBUF_SIZE,(FTU8 *)mcuCMDBuf,mcuCMDBufSize));
...
FTVOID HAL_DlpBufIn (FTU32 Dlp)
{
	if (mcuCMDBuf) {
		...
		mcuCMDindex += FTU32_LEN;
		if (mcuCMDindex == mcuCMDBufSize) {
			HAL_Write8Src(RAM_DL+mcuDLindex,(FTU8 *)mcuCMDBuf,mcuCMDBufSize);
			mcuCMDindex = 0;
...
FTVOID HAL_BufToReg (FTU32 reg, FTU32 padNum)
{
	if (reg == RAM_CMD) {
		if (mcuCMDBuf) {
			...
			HAL_CmdWait((FTU16)HAL_EVELoopMemWr(reg,HAL_Read32(REG_CMD_WRITE),CMDBUF_SIZE,(FTU8 *)mcuCMDBuf,mcuCMDindex));
		} else {
...
FTVOID HAL_FT800_TouchConfig ( FTVOID )
{
	...
	FTDELAY(300);
	HAL_Write16(REG_CYA_TOUCH,(HAL_Read16(REG_CYA_TOUCH) & 0x7fff)); //capacitive touch
	FTDELAY(300);
...
#else
/*for this code, I consider "resistance touch" + "WVGA" means using FTDI WH70R
  WH70R use default touch threshold: the lightest touch will be accepted
  you may change it depends on your real system*/
#if !defined(LCD_WVGA)
    HAL_Write16(REG_TOUCH_RZTHRESH,FT800_TOUCH_THRESHOLD);
#endif
	HAL_Write16(REG_CYA_TOUCH,(HAL_Read16(REG_CYA_TOUCH) | 0x8000)); //resistive touch
#endif
}
...
{
	while (1) {
		HAL_CmdBufIn(CMD_DLSTART);               //4
		HAL_CmdBufIn(CLEAR_COLOR_RGB(0xFF,0,0)); //4
		HAL_CmdBufIn(CLEAR(1,1,1));              //4

		CoCmd_TEXT(FT800_LCD_WIDTH/2,FT800_LCD_HIGH/3,24,OPT_CENTERX,"Missing correct files in SD card");
		//4*4
		CoCmd_TEXT(FT800_LCD_WIDTH/2,FT800_LCD_HIGH/3*2,24,OPT_CENTERX,"or image not for this platform");
		//4*4

		HAL_CmdBufIn(DISPLAY()); //4
		HAL_CmdBufIn(CMD_SWAP);  //4
...
FTVOID HAL_FT800_BootupDisp ( FTU32 count )
{
	...
	do {
		...
		switch (EVE_ID) {
			case 0x10:
				CoCmd_TEXT(FT800_LCD_WIDTH/3,FT800_LCD_HIGH/4,VER_FONT,OPT_CENTERX,"FT810");
				break;
			case 0x11:
				CoCmd_TEXT(FT800_LCD_WIDTH/3,FT800_LCD_HIGH/4,VER_FONT,OPT_CENTERX,"FT811");
				break;
			case 0x12:
				CoCmd_TEXT(FT800_LCD_WIDTH/3,FT800_LCD_HIGH/4,VER_FONT,OPT_CENTERX,"FT812");
				break;
			case 0x13:
				CoCmd_TEXT(FT800_LCD_WIDTH/3,FT800_LCD_HIGH/4,VER_FONT,OPT_CENTERX,"FT813");
				break;
			default:
				/* only new FT81X able to read the EVE ID */
				CoCmd_TEXT(FT800_LCD_WIDTH/3,FT800_LCD_HIGH/4,VER_FONT,OPT_CENTERX,"EVE Chip");
				break;
		}
#if defined(DEF_CAP_MULTI)
		CoCmd_TEXT(FT800_LCD_WIDTH/3*2,FT800_LCD_HIGH/4,VER_FONT,OPT_CENTERX,"CAP-M");
#elif defined(DEF_CAP_NONMULTI)
		CoCmd_TEXT(FT800_LCD_WIDTH/3*2,FT800_LCD_HIGH/4,VER_FONT,OPT_CENTERX,"CAP-NM");
#else
		CoCmd_TEXT(FT800_LCD_WIDTH/3*2,FT800_LCD_HIGH/4,VER_FONT,OPT_CENTERX,"RES");
#endif
		CoCmd_TEXT(FT800_LCD_WIDTH/2,FT800_LCD_HIGH/2,VER_FONT,OPT_CENTERX,"Ver: "FT800_VER);
		if (mcuCMDBuf) {
			CoCmd_TEXT(FT800_LCD_WIDTH/2-10,FT800_LCD_HIGH/4*3,CAL_FONT,OPT_CENTERX,"MCU CMD Buf: ");
			CoCmd_NUMBER(FT800_LCD_WIDTH/2+40,FT800_LCD_HIGH/4*3,CAL_FONT,OPT_CENTERX,mcuCMDBufSize);
		}
#if defined(STM32F4)
		CoCmd_TEXT(FT800_LCD_WIDTH/2-30,FT800_LCD_HIGH/4*3+10,CAL_FONT,OPT_CENTERX,"File addr: ");
		CoCmd_NUMBER(FT800_LCD_WIDTH/2+40,FT800_LCD_HIGH/4*3+10,CAL_FONT,OPT_CENTERX,FILE_SADDR);
#endif
		if (SYS_HANG) {
			CoCmd_TEXT(FT800_LCD_WIDTH/2,FT800_LCD_HIGH - CAL_WIDE*2,CAL_FONT,OPT_CENTERX,"system hange due to memory limit!");
		} else {
#if defined(CAL_NEEDED)
			CoCmd_TEXT(FT800_LCD_WIDTH/2,FT800_LCD_HIGH - CAL_WIDE*2,CAL_FONT,OPT_CENTERX,"press the screen for a while to force in calibration");
#else
			CoCmd_TEXT(FT800_LCD_WIDTH/2,FT800_LCD_HIGH - CAL_WIDE*2,CAL_FONT,OPT_CENTERX,"no calibration in this demo");
#endif
			CoCmd_NUMBER(FT800_LCD_WIDTH/2,FT800_LCD_HIGH - CAL_WIDE,CAL_FONT,OPT_CENTERX,count);
		}
