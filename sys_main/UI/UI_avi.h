...
#define AVI_FIFOSIZE (EVE_RAMG_SIZE - AVI_FIFOADDR)
...
STATIC FTVOID Display (FTU32 opt, FTU32 start, FTU32 frame, FTU32 fIndex, FTU32 fLen)
{
	...
	if (OPT_MEDIAFIFO&opt) {
		...
		if (start) {
			...
			CoCmd_LOADIDENTITY;
			CoCmd_SCALE(EVE_TRANSFORM_MAX/2,EVE_TRANSFORM_MAX/2);
			CoCmd_SETMATRIX;
			
			HAL_CmdBufIn(VERTEX2F(0,0));
			HAL_CmdBufIn(VERTEX2F(AVI_FRAME_WIDE/2*EVE_PIXEL_UNIT,AVI_FRAME_HIGH/2*EVE_PIXEL_UNIT));
			HAL_CmdBufIn(VERTEX2F(AVI_FRAME_WIDE/4*EVE_PIXEL_UNIT,AVI_FRAME_HIGH/4*EVE_PIXEL_UNIT));
