...
STATIC FTVOID coobot_speed (FTVOID)
{
...
STATIC FTVOID coobot_info (FTVOID)
{
	HAL_CmdBufIn(T_COLOR);
...
STATIC FTVOID coobot_filling (FTVOID)
{
...
STATIC FTVOID coobot_Zoomin (FTU8 handle, FT32 X, FT32 Y, FT32 W, FT32 H)
{
	FT32 step;
	...
	for (step = 1; step <= LOGO_ZOOM; step++) {
		...
		CoCmd_SCALE(EVE_TRANSFORM_MAX/LOGO_ZOOM*step,EVE_TRANSFORM_MAX/LOGO_ZOOM*step);
		CoCmd_TRANSLATE(-W/2*EVE_TRANSFORM_MAX,-H/2*EVE_TRANSFORM_MAX);
		CoCmd_SETMATRIX;
...
STATIC FTVOID coobot_Gradient(FTVOID)
{
	FT32 Y;
...
{	
	...
	coobot_Zoomin(0, EVE_LCD_WIDTH/2-211/2, EVE_LCD_HIGH/2-221/2, 211, 221);
