...
#define TXT_ULCK_X   (FT800_LCD_WIDTH/2-TXT_ULCK_W/2)
#define TXT_ULCK_Y   (FT800_LCD_HIGH/3-TXT_ULCK_H/2)
#define BG_ULCK_W   360
#define BG_ULCK_H   96
#define BG_ULCK_X   (FT800_LCD_WIDTH/2-BG_ULCK_W/2)
#define BG_ULCK_Y   (FT800_LCD_HIGH/2-BG_ULCK_H/2)
...
#define ICON_ULCK_Y   (FT800_LCD_HIGH/2-ICON_ULCK_H/2)
#define TAG_LCK     10
#define GO_CLK      300
	enum bmp_hdl_ {
		ULCK_TXT_HDL = 0,
	...
	while (1) {
		...
		HAL_CmdBufIn(VERTEX2F(TXT_ULCK_X*FT800_PIXEL_UNIT,TXT_ULCK_Y*FT800_PIXEL_UNIT));
		HAL_CmdBufIn(BITMAP_HANDLE(ULCK_BG_HDL));
		HAL_CmdBufIn(VERTEX2F(BG_ULCK_X*FT800_PIXEL_UNIT,BG_ULCK_Y*FT800_PIXEL_UNIT));

		HAL_CmdBufIn(TAG_MASK(1));
		HAL_CmdBufIn(TAG(TAG_LCK));
		if (locked) {
			HAL_CmdBufIn(BITMAP_HANDLE(lck_hdl));
			HAL_CmdBufIn(VERTEX2F(lck_x,ICON_ULCK_Y*FT800_PIXEL_UNIT));
		}
		HAL_CmdBufIn(TAG_MASK(0));
		HAL_CmdBufIn(BITMAP_HANDLE(opn_hdl));
		HAL_CmdBufIn(VERTEX2F(R_ICON_ULCK_X*FT800_PIXEL_UNIT,ICON_ULCK_Y*FT800_PIXEL_UNIT));
		HAL_CmdBufIn(END());
...
#define M_MID_W   148
#define M_MID_H   148
#define M_TOP_LCK_X  (FT800_LCD_WIDTH/32)
#define M_TOP_SET_X  (FT800_LCD_WIDTH/32*31-M_TOP_W)
#define M_TOP_Y  (FT800_LCD_HIGH/8)
#define M_BTM_GAP    70
#define M_BTM_WEK_X  (FT800_LCD_WIDTH/2-M_BTM_W/2)
...
#define M_BTM_Y  (FT800_LCD_HIGH/4*3-20)
...
#define M_MID_GAP    50
#define M_MID_AIR_X    (FT800_LCD_WIDTH/2-M_MID_GAP/2-M_MID_W)
#define M_MID_NAV_X    (FT800_LCD_WIDTH/2+M_MID_GAP/2)
#define M_MID_Y    (FT800_LCD_HIGH/2-M_MID_H/2-20)
...
#define VIBERATE_REMOVE 250
	...
	while (1) {
		...
		HAL_CmdBufIn(VERTEX2F(M_TOP_LCK_X*FT800_PIXEL_UNIT,M_TOP_Y*FT800_PIXEL_UNIT));
		HAL_CmdBufIn(TAG(M_TAG_SET));
		HAL_CmdBufIn(CELL(M_TOP_SET_CELL));
		HAL_CmdBufIn(VERTEX2F(M_TOP_SET_X*FT800_PIXEL_UNIT,M_TOP_Y*FT800_PIXEL_UNIT));
		...
		for (i = 0; i < sizeof(hdl_btm); i++) {
			if (hdl_btm[i] == M_BTM1_HDL) {
				...
				switch (i+M_TAG_ALM) {
					case M_TAG_ALM:
						CoCmd_TEXT((FT800_LCD_WIDTH/2),(FT800_LCD_HIGH/2),25,OPT_CENTERX,"Alarm UI, not finished");
						break;
					case M_TAG_LGT:
						CoCmd_TEXT((FT800_LCD_WIDTH/2),(FT800_LCD_HIGH/2),25,OPT_CENTERX,"Light UI, not finished");
						break;
					case M_TAG_WEK:
						CoCmd_TEXT((FT800_LCD_WIDTH/2),(FT800_LCD_HIGH/2),25,OPT_CENTERX,"Weak UI, not finished");
						break;
					case M_TAG_STG:
						CoCmd_TEXT((FT800_LCD_WIDTH/2),(FT800_LCD_HIGH/2),25,OPT_CENTERX,"Strong UI, not finished");
						break;
					case M_TAG_PWR:
						CoCmd_TEXT((FT800_LCD_WIDTH/2),(FT800_LCD_HIGH/2),25,OPT_CENTERX,"Power off UI, not finished");
						break;
					default:
						i = sizeof(hdl_btm);
						break;
		...
		if (i == sizeof(hdl_btm)) {
			...
			HAL_CmdBufIn(VERTEX2F(M_MID_AIR_X*FT800_PIXEL_UNIT,M_MID_Y*FT800_PIXEL_UNIT));
			HAL_CmdBufIn(TAG(M_TAG_NAV));
			HAL_CmdBufIn(CELL(M_MID_NAV_CELL));
			HAL_CmdBufIn(VERTEX2F(M_MID_NAV_X*FT800_PIXEL_UNIT,M_MID_Y*FT800_PIXEL_UNIT));
		}
		...
		HAL_CmdBufIn(CELL(M_BTM_ALM_CELL));
		HAL_CmdBufIn(VERTEX2F(M_BTMt_ALM_X*FT800_PIXEL_UNIT,M_BTMt_Y*FT800_PIXEL_UNIT));

		HAL_CmdBufIn(CELL(M_BTM_LGT_CELL));
		HAL_CmdBufIn(VERTEX2F(M_BTMt_LGT_X*FT800_PIXEL_UNIT,M_BTMt_Y*FT800_PIXEL_UNIT));
		
		HAL_CmdBufIn(CELL(M_BTM_WEK_CELL));
		HAL_CmdBufIn(VERTEX2F(M_BTMt_WEK_X*FT800_PIXEL_UNIT,M_BTMt_Y*FT800_PIXEL_UNIT));
	
		HAL_CmdBufIn(CELL(M_BTM_STG_CELL));
		HAL_CmdBufIn(VERTEX2F(M_BTMt_STG_X*FT800_PIXEL_UNIT,M_BTMt_Y*FT800_PIXEL_UNIT));
	
		HAL_CmdBufIn(CELL(M_BTM_PWR_CELL));
		HAL_CmdBufIn(VERTEX2F(M_BTMt_PWR_X*FT800_PIXEL_UNIT,M_BTMt_Y*FT800_PIXEL_UNIT));
...
#define CLK_1_X   (FT800_LCD_WIDTH/2-CLK_2_W/2-CLK_1_W)
#define CLK_1_Y   (FT800_LCD_HIGH/2-CLK_1_H/2)
#define CLK_2_X   (FT800_LCD_WIDTH/2-CLK_2_W/2)
#define CLK_2_Y   (CLK_1_Y-CLK_2_H/2)

#define CLK_M_W   267
#define CLK_M_H   267
#define CLK_M_X   (FT800_LCD_WIDTH/2-CLK_M_W/2)
#define CLK_M_Y   (FT800_LCD_HIGH/4-15)

#define CLK_H_W   178
#define CLK_H_H   178
#define CLK_H_X   (FT800_LCD_WIDTH/2-CLK_H_W/2)
#define CLK_H_Y   (FT800_LCD_HIGH/4+28)
...
#define CLK_ANGLE (360/60)
	...
	while (1) {
		...
		HAL_CmdBufIn(VERTEX2F(CLK_1_X*FT800_PIXEL_UNIT,CLK_1_Y*FT800_PIXEL_UNIT));
		HAL_CmdBufIn(BITMAP_HANDLE(CLK_2_HDL));
		HAL_CmdBufIn(VERTEX2F(CLK_2_X*FT800_PIXEL_UNIT,CLK_2_Y*FT800_PIXEL_UNIT));
		HAL_CmdBufIn(END());

		/* rotate left 180 degree put at right */
		HAL_CmdBufIn(BEGIN(BITMAPS));
		CoCmd_LOADIDENTITY;	      
		CoCmd_TRANSLATE(CLK_1_W/2*FT800_TRANSFORM_MAX,CLK_1_H/2*FT800_TRANSFORM_MAX);
		CoCmd_ROTATE(180*FT800_TRANSFORM_MAX/360);
		CoCmd_TRANSLATE(-CLK_1_W/2*FT800_TRANSFORM_MAX,-CLK_1_H/2*FT800_TRANSFORM_MAX);
		CoCmd_SETMATRIX;
		HAL_CmdBufIn(BITMAP_HANDLE(CLK_1_HDL));
		HAL_CmdBufIn(VERTEX2F((CLK_2_X+CLK_2_W)*FT800_PIXEL_UNIT,(CLK_1_Y)*FT800_PIXEL_UNIT));
		SETDEFAULT;
		HAL_CmdBufIn(END());
		...
		HAL_CmdBufIn(BEGIN(BITMAPS));
		CoCmd_LOADIDENTITY;	      
		CoCmd_TRANSLATE(CLK_2_W/2*FT800_TRANSFORM_MAX,CLK_2_H/2*FT800_TRANSFORM_MAX);
		CoCmd_ROTATE(180*FT800_TRANSFORM_MAX/360);
		CoCmd_TRANSLATE(-CLK_2_W/2*FT800_TRANSFORM_MAX,-CLK_2_H/2*FT800_TRANSFORM_MAX);
		CoCmd_SETMATRIX;
		HAL_CmdBufIn(BITMAP_HANDLE(CLK_2_HDL));
		HAL_CmdBufIn(VERTEX2F(CLK_2_X*FT800_PIXEL_UNIT,(CLK_1_Y+CLK_1_H)*FT800_PIXEL_UNIT));
		SETDEFAULT;
		HAL_CmdBufIn(END());
		...
		HAL_CmdBufIn(BEGIN(BITMAPS));
		CoCmd_LOADIDENTITY;
		CoCmd_TRANSLATE(CLK_M_W/2*FT800_TRANSFORM_MAX,CLK_M_H/2*FT800_TRANSFORM_MAX);
		CoCmd_ROTATE(r_m*FT800_TRANSFORM_MAX/360);
		CoCmd_TRANSLATE(-CLK_M_W/2*FT800_TRANSFORM_MAX,-CLK_M_H/2*FT800_TRANSFORM_MAX);
		CoCmd_SETMATRIX;
		HAL_CmdBufIn(BITMAP_HANDLE(CLK_M_HDL));
		HAL_CmdBufIn(VERTEX2F(CLK_M_X*FT800_PIXEL_UNIT,CLK_M_Y*FT800_PIXEL_UNIT));
		SETDEFAULT;
		HAL_CmdBufIn(END());
		...
		HAL_CmdBufIn(BEGIN(BITMAPS));
		CoCmd_LOADIDENTITY;
		CoCmd_TRANSLATE(CLK_H_W/2*FT800_TRANSFORM_MAX,CLK_H_H/2*FT800_TRANSFORM_MAX);
		CoCmd_ROTATE(r_h*FT800_TRANSFORM_MAX/360);
		CoCmd_TRANSLATE(-CLK_H_W/2*FT800_TRANSFORM_MAX,-CLK_H_H/2*FT800_TRANSFORM_MAX);
		CoCmd_SETMATRIX;
		HAL_CmdBufIn(BITMAP_HANDLE(CLK_H_HDL));
		HAL_CmdBufIn(VERTEX2F(CLK_H_X*FT800_PIXEL_UNIT,CLK_H_Y*FT800_PIXEL_UNIT));
		SETDEFAULT;
		HAL_CmdBufIn(END());
