...
STATIC FTVOID dispButm (FTU8 HomeOK, FTU8 Pressed)
{
	...
	HAL_CmdBufIn(VERTEX2F(0,(LCD_H-BUTM_H)*FT800_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F(BUTM_W*FT800_PIXEL_UNIT,LCD_H*FT800_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F((LCD_W-BUTM_W)*FT800_PIXEL_UNIT,(LCD_H-BUTM_H)*FT800_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F(LCD_W*FT800_PIXEL_UNIT,LCD_H*FT800_PIXEL_UNIT));
	HAL_CmdBufIn(END());
    ...
    CoCmd_SCALE(HomeR*FT800_TRANSFORM_MAX/100,HomeR*FT800_TRANSFORM_MAX/100);
    CoCmd_TRANSLATE(HOME_W/2*FT800_TRANSFORM_MAX*(-1), HOME_H/2*FT800_TRANSFORM_MAX*(-1));
	CoCmd_SETMATRIX;
	HAL_CmdBufIn(VERTEX2F(((BUTM_W-HOME_W)/2)*FT800_PIXEL_UNIT,
                         ((LCD_H-BUTM_H)+(BUTM_H-HOME_H)/2)*FT800_PIXEL_UNIT));
    HAL_CmdBufIn(RESTORE_CONTEXT());
	...
	CoCmd_LOADIDENTITY;
    CoCmd_TRANSLATE(HOME_W/2*FT800_TRANSFORM_MAX, HOME_H/2*FT800_TRANSFORM_MAX);
    CoCmd_SCALE(OKR*FT800_TRANSFORM_MAX/100,OKR*FT800_TRANSFORM_MAX/100);
    CoCmd_TRANSLATE(HOME_W/2*FT800_TRANSFORM_MAX*(-1), HOME_H/2*FT800_TRANSFORM_MAX*(-1));
	CoCmd_SETMATRIX;
	HAL_CmdBufIn(VERTEX2F(((LCD_W-BUTM_W)+(BUTM_W-OK_W)/2)*FT800_PIXEL_UNIT,
                         ((LCD_H-BUTM_H)+(BUTM_H-OK_H)/2)*FT800_PIXEL_UNIT));
    HAL_CmdBufIn(RESTORE_CONTEXT());
    HAL_CmdBufIn(END());
...
STATIC FTVOID dispArrow (FTU8 LeftRight, FTU8 Pressed)
{
    ...
    CoCmd_SCALE(LeftR*FT800_TRANSFORM_MAX/100,LeftR*FT800_TRANSFORM_MAX/100);
    CoCmd_TRANSLATE(LEFT_W/2*FT800_TRANSFORM_MAX*(-1), LEFT_H/2*FT800_TRANSFORM_MAX*(-1));
	CoCmd_SETMATRIX;
	HAL_CmdBufIn(VERTEX2F(0*FT800_PIXEL_UNIT,
                         (LCD_H/2-(LEFT_H)/2)*FT800_PIXEL_UNIT));
	HAL_CmdBufIn(RESTORE_CONTEXT());
    ...
    CoCmd_TRANSLATE(LEFT_W/2*FT800_TRANSFORM_MAX*(-1), LEFT_H/2*FT800_TRANSFORM_MAX*(-1));
	CoCmd_SETMATRIX;
	HAL_CmdBufIn(VERTEX2F((LCD_W-LEFT_W)*FT800_PIXEL_UNIT,
                         (LCD_H/2-(LEFT_H)/2)*FT800_PIXEL_UNIT));
    HAL_CmdBufIn(RESTORE_CONTEXT());
    HAL_CmdBufIn(END());
	HAL_CmdBufIn(TAG_MASK(0));
...
#define TXTBKG_GAP  2
    ...
    HAL_CmdBufIn(POINT_SIZE(TXTBKG_P_SZ*FT800_PIXEL_UNIT));
    HAL_CmdBufIn(BEGIN(FTPOINTS));
	HAL_CmdBufIn(VERTEX2F(X*FT800_PIXEL_UNIT,Y*FT800_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F((X+TXTBKG_L_LN)*FT800_PIXEL_UNIT,Y*FT800_PIXEL_UNIT));
    HAL_CmdBufIn(END());
	HAL_CmdBufIn(BEGIN(RECTS));
	HAL_CmdBufIn(VERTEX2F(X*FT800_PIXEL_UNIT,(Y-TXTBKG_P_SZ+1)*FT800_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F((X+TXTBKG_L_LN)*FT800_PIXEL_UNIT,(Y+TXTBKG_P_SZ-1)*FT800_PIXEL_UNIT));
    HAL_CmdBufIn(END());
    if (!Selected) {
        HAL_CmdBufIn(COLOR_RGB(BKG_R,BKG_G,BKG_B));
        HAL_CmdBufIn(POINT_SIZE((TXTBKG_P_SZ-TXTBKG_GAP)*FT800_PIXEL_UNIT));
        HAL_CmdBufIn(BEGIN(FTPOINTS));
        HAL_CmdBufIn(VERTEX2F(X*FT800_PIXEL_UNIT,Y*FT800_PIXEL_UNIT));
        HAL_CmdBufIn(VERTEX2F((X+TXTBKG_L_LN)*FT800_PIXEL_UNIT,Y*FT800_PIXEL_UNIT));
        HAL_CmdBufIn(END());
        HAL_CmdBufIn(BEGIN(RECTS));
        HAL_CmdBufIn(VERTEX2F(X*FT800_PIXEL_UNIT,(Y-TXTBKG_P_SZ+TXTBKG_GAP+1)*FT800_PIXEL_UNIT));
        HAL_CmdBufIn(VERTEX2F((X+TXTBKG_L_LN)*FT800_PIXEL_UNIT,(Y+TXTBKG_P_SZ-TXTBKG_GAP-1)*FT800_PIXEL_UNIT));
        HAL_CmdBufIn(END());
    }
}
...
STATIC FTVOID dispDateNumber (FTU32 X, FTU32 Y, FTU32 number, FTU32 ratio)
{
	...
	CoCmd_LOADIDENTITY;
    CoCmd_TRANSLATE(LEFT_W/2*FT800_TRANSFORM_MAX, LEFT_H/2*FT800_TRANSFORM_MAX);
    CoCmd_SCALE(FT800_TRANSFORM_MAX,ratio*FT800_TRANSFORM_MAX/100); /* only change the Y direction */
    CoCmd_TRANSLATE(LEFT_W/2*FT800_TRANSFORM_MAX*(-1), LEFT_H/2*FT800_TRANSFORM_MAX*(-1));
	CoCmd_SETMATRIX;
...
#define MASK_ENLARGE ((DATEDAY_X + DATEDAY_W - DATEYEAR_X)/MASK_W)
    ...
    HAL_CmdBufIn(VERTEX2F(DATEYEAR_X*FT800_PIXEL_UNIT,DATE_NUM_Y*FT800_PIXEL_UNIT));
    HAL_CmdBufIn(VERTEX2F((DATEYEAR_X+DATEYEAR_W)*FT800_PIXEL_UNIT,(DATE_NUM_Y+DATE_H)*FT800_PIXEL_UNIT));
    
    HAL_CmdBufIn(TAG(TAG_MONTH));
    HAL_CmdBufIn(VERTEX2F(DATEMONTH_X*FT800_PIXEL_UNIT,DATE_NUM_Y*FT800_PIXEL_UNIT));
    HAL_CmdBufIn(VERTEX2F((DATEMONTH_X+DATEMONTH_W)*FT800_PIXEL_UNIT,(DATE_NUM_Y+DATE_H)*FT800_PIXEL_UNIT));
   
    HAL_CmdBufIn(TAG(TAG_DAY));
    HAL_CmdBufIn(VERTEX2F(DATEDAY_X*FT800_PIXEL_UNIT,DATE_NUM_Y*FT800_PIXEL_UNIT));
    HAL_CmdBufIn(VERTEX2F((DATEDAY_X+DATEDAY_W)*FT800_PIXEL_UNIT,(DATE_NUM_Y+DATE_H)*FT800_PIXEL_UNIT));
