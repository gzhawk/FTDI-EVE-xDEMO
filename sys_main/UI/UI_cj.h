...
#define CJ_FIFOADDR        (EVE_RAMG_SIZE - CJ_FIFOSIZE)

#define BUTN_SWT_X 0
#define BUTN_PWM_X (EVE_LCD_WIDTH/2)
#define BUTN_Y 0
#define BUTN_W (EVE_LCD_WIDTH/2)
#define BUTN_H (EVE_LCD_HIGH/16)
#define BUTN_F (30)
...
#define PWM_MOTO_GAP (EVE_LCD_WIDTH/6)
#define PWM_LINE_GAP (EVE_LCD_WIDTH/3)
#define PWM_COLOR    HAL_CmdBufIn(COLOR_RGB(109, 115, 115))
#define PWM_SWITCH_BG CoCmd_BGCOLOR((109<<16)|(115<<8)|115)
#define PWM_F (23)
#define PWM_SLIDER_H (EVE_LCD_HIGH/32)
...
FTVOID cj_swt_one_led (FTU8 off_on, FTU8 ra)
{
	...
	HAL_CmdBufIn(POINT_SIZE(ra*EVE_PIXEL_UNIT));
}
...
#define SWT_LED_SPEED 10
	...
	HAL_CmdBufIn(VERTEX2F((x+SWT_LED_LOBBY_X)*EVE_PIXEL_UNIT,
						  (y+SWT_LED_LOBBY_Y)*EVE_PIXEL_UNIT));

	HAL_CmdBufIn(TAG(eT_SWT_BED));
	cj_swt_one_led(home_sw.bed,home_sw.bed?Ra:SWT_LED_RA);
	HAL_CmdBufIn(VERTEX2F((x+SWT_LED_BED_X)*EVE_PIXEL_UNIT,
						  (y+SWT_LED_BED_Y)*EVE_PIXEL_UNIT));

	HAL_CmdBufIn(TAG(eT_SWT_BED1));
	cj_swt_one_led(home_sw.bed1,home_sw.bed1?Ra:SWT_LED_RA);
	HAL_CmdBufIn(VERTEX2F((x+SWT_LED_BED1_X)*EVE_PIXEL_UNIT,
						  (y+SWT_LED_BED1_Y)*EVE_PIXEL_UNIT));

	HAL_CmdBufIn(TAG(eT_SWT_BED2));
	cj_swt_one_led(home_sw.bed2,home_sw.bed2?Ra:SWT_LED_RA);
	HAL_CmdBufIn(VERTEX2F((x+SWT_LED_BED2_X)*EVE_PIXEL_UNIT,
						  (y+SWT_LED_BED2_Y)*EVE_PIXEL_UNIT));

	HAL_CmdBufIn(TAG(eT_SWT_KITCHEN));
	cj_swt_one_led(home_sw.kitchen,home_sw.kitchen?Ra:SWT_LED_RA);
	HAL_CmdBufIn(VERTEX2F((x+SWT_LED_KD_K_X)*EVE_PIXEL_UNIT,
						  (y+SWT_LED_KD_Y)*EVE_PIXEL_UNIT));

	HAL_CmdBufIn(TAG(eT_SWT_DINNER));
	cj_swt_one_led(home_sw.dinner,home_sw.dinner?Ra:SWT_LED_RA);
	HAL_CmdBufIn(VERTEX2F((x+SWT_LED_KD_D_X)*EVE_PIXEL_UNIT,
						  (y+SWT_LED_KD_Y)*EVE_PIXEL_UNIT));

	HAL_CmdBufIn(TAG(eT_SWT_WC));
	cj_swt_one_led(home_sw.wc,home_sw.wc?Ra:SWT_LED_RA);
	HAL_CmdBufIn(VERTEX2F((x+SWT_LED_WC_X)*EVE_PIXEL_UNIT,
						  (y+SWT_LED_WC_Y)*EVE_PIXEL_UNIT));
...
{
	FTU32 x = ((EVE_LCD_WIDTH-SWT_PIC_W)/2), y = ((EVE_LCD_HIGH-SWT_PIC_H)/2);
...
FTVOID cj_pwm_motor (FTVOID)
{
	...
	HAL_CmdBufIn(BEGIN(BITMAPS));
	HAL_CmdBufIn(VERTEX2F((PWM_MOTO_GAP-PWM_PIC_W)*EVE_PIXEL_UNIT,(EVE_LCD_HIGH-PWM_PIC_H)*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F((PWM_MOTO_GAP*3-PWM_PIC_W)*EVE_PIXEL_UNIT,(EVE_LCD_HIGH-PWM_PIC_H)*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F((PWM_MOTO_GAP*5-PWM_PIC_W)*EVE_PIXEL_UNIT,(EVE_LCD_HIGH-PWM_PIC_H)*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(END());
	...
	CoCmd_TOGGLE(PWM_MOTO_GAP+30,EVE_LCD_HIGH - BUTN_H-10,PWM_MOTO_GAP/2,
				 PWM_F,0,motor_pwm.pwm1.motor_off_on,"Start""\xFF""   Stop");
	HAL_CmdBufIn(TAG(eT_PWM2_SW));
	CoCmd_TOGGLE(PWM_MOTO_GAP*3+30,EVE_LCD_HIGH - BUTN_H-10,PWM_MOTO_GAP/2,
				 PWM_F,0,motor_pwm.pwm2.motor_off_on,"Start""\xFF""   Stop");
	HAL_CmdBufIn(TAG(eT_PWM3_SW));
	CoCmd_TOGGLE(PWM_MOTO_GAP*5+30,EVE_LCD_HIGH - BUTN_H-10,PWM_MOTO_GAP/2,
...
FTVOID cj_pwm_lines (FTVOID)
{
	...
	HAL_CmdBufIn(VERTEX2F(PWM_LINE_GAP*EVE_PIXEL_UNIT,BUTN_H*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F(PWM_LINE_GAP*EVE_PIXEL_UNIT,EVE_LCD_HIGH*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F(PWM_LINE_GAP*2*EVE_PIXEL_UNIT,BUTN_H*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F(PWM_LINE_GAP*2*EVE_PIXEL_UNIT,EVE_LCD_HIGH*EVE_PIXEL_UNIT));

	/* wave form */
	/* wave form 1.1 high*/
	HAL_CmdBufIn(VERTEX2F(PWM_SLIDER_GAP*EVE_PIXEL_UNIT, WAVE_HIGH_Y*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F(WAVE_1_X*EVE_PIXEL_UNIT, WAVE_HIGH_Y*EVE_PIXEL_UNIT));

	/* wave form 1.1 low*/
	HAL_CmdBufIn(VERTEX2F(WAVE_1_X*EVE_PIXEL_UNIT, WAVE_LOW_Y*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F((PWM_SLIDER_GAP+PWM_SLIDER_W)*EVE_PIXEL_UNIT, WAVE_LOW_Y*EVE_PIXEL_UNIT));

	/* wave form 2.1 high*/
	HAL_CmdBufIn(VERTEX2F((PWM_LINE_GAP+PWM_SLIDER_GAP)*EVE_PIXEL_UNIT,
						  WAVE_HIGH_Y*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F(WAVE_2_X*EVE_PIXEL_UNIT, WAVE_HIGH_Y*EVE_PIXEL_UNIT));

	/* wave form 2.1 low*/
	HAL_CmdBufIn(VERTEX2F(WAVE_2_X*EVE_PIXEL_UNIT, WAVE_LOW_Y*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F((PWM_LINE_GAP+PWM_SLIDER_GAP+PWM_SLIDER_W)*EVE_PIXEL_UNIT,
						  WAVE_LOW_Y*EVE_PIXEL_UNIT));

	/* wave form 3.1 high*/
	HAL_CmdBufIn(VERTEX2F((PWM_LINE_GAP*2+PWM_SLIDER_GAP)*EVE_PIXEL_UNIT,
						  (PWM_RATIO_NUM_Y+RATIO_WAVE_ON)*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F(WAVE_3_X*EVE_PIXEL_UNIT, (PWM_RATIO_NUM_Y+RATIO_WAVE_ON)*EVE_PIXEL_UNIT));

	/* wave form 3.1 low*/
	HAL_CmdBufIn(VERTEX2F(WAVE_3_X*EVE_PIXEL_UNIT, (PWM_RATIO_NUM_Y+RATIO_MARK_H)*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F((PWM_LINE_GAP*2+PWM_SLIDER_GAP+PWM_SLIDER_W)*EVE_PIXEL_UNIT,
						  (PWM_RATIO_NUM_Y+RATIO_MARK_H)*EVE_PIXEL_UNIT));

	/* duty ratio */
	HAL_CmdBufIn(COLOR_RGB(255, 0, 0));
	HAL_CmdBufIn(VERTEX2F(WAVE_1_X*EVE_PIXEL_UNIT,
						  (PWM_RATIO_NUM_Y+RATIO_MARK)*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F(WAVE_1_X*EVE_PIXEL_UNIT,
						  (PWM_RATIO_NUM_Y+RATIO_MARK_H)*EVE_PIXEL_UNIT));

	HAL_CmdBufIn(VERTEX2F(WAVE_2_X*EVE_PIXEL_UNIT,
						  (PWM_RATIO_NUM_Y+RATIO_MARK)*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F(WAVE_2_X*EVE_PIXEL_UNIT,
						  (PWM_RATIO_NUM_Y+RATIO_MARK_H)*EVE_PIXEL_UNIT));

	HAL_CmdBufIn(VERTEX2F(WAVE_3_X*EVE_PIXEL_UNIT,
						  (PWM_RATIO_NUM_Y+RATIO_MARK)*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F(WAVE_3_X*EVE_PIXEL_UNIT,
						  (PWM_RATIO_NUM_Y+RATIO_MARK_H)*EVE_PIXEL_UNIT));
...
FTVOID cj_pwm_slider (FTVOID)
{
	...
	if (!motor_pwm.pwm1.motor_off_on) {
		...
		CoCmd_SLIDER(PWM_SLIDER_GAP, EVE_LCD_HIGH/3*2, PWM_SLIDER_W, PWM_SLIDER_H, 0, motor_pwm.pwm1.duty_ratio, 100);
		CoCmd_TRACK(PWM_SLIDER_GAP, EVE_LCD_HIGH/3*2, PWM_SLIDER_W, 1, eT_PWM1_SLDR);
	} else {
		CoCmd_TEXT(PWM_LINE_GAP/2, EVE_LCD_HIGH/3*2, PWM_F, OPT_CENTERX, "Duty Ratio (1%~99%)");
	}
	if (!motor_pwm.pwm2.motor_off_on) {
		HAL_CmdBufIn(TAG(eT_PWM2_SLDR));
		CoCmd_SLIDER(PWM_LINE_GAP+PWM_SLIDER_GAP, EVE_LCD_HIGH/3*2, PWM_SLIDER_W, PWM_SLIDER_H, 0, motor_pwm.pwm2.duty_ratio, 100);
		CoCmd_TRACK(PWM_LINE_GAP+PWM_SLIDER_GAP, EVE_LCD_HIGH/3*2, PWM_SLIDER_W, 1, eT_PWM2_SLDR);
	} else {
		CoCmd_TEXT(PWM_LINE_GAP+PWM_LINE_GAP/2, EVE_LCD_HIGH/3*2, PWM_F, OPT_CENTERX, "Duty Ratio (1%~99%)");
	}
	if (!motor_pwm.pwm3.motor_off_on) {
		HAL_CmdBufIn(TAG(eT_PWM3_SLDR));
		CoCmd_SLIDER(PWM_LINE_GAP*2+PWM_SLIDER_GAP, EVE_LCD_HIGH/3*2, PWM_SLIDER_W, PWM_SLIDER_H, 0, motor_pwm.pwm3.duty_ratio, 100);
		CoCmd_TRACK(PWM_LINE_GAP*2+PWM_SLIDER_GAP, EVE_LCD_HIGH/3*2, PWM_SLIDER_W, 1, eT_PWM3_SLDR);
	} else {
		CoCmd_TEXT(PWM_LINE_GAP*2+PWM_LINE_GAP/2, EVE_LCD_HIGH/3*2, PWM_F, OPT_CENTERX, "Duty Ratio (1%~99%)");
	}
	HAL_CmdBufIn(TAG_MASK(0));
