...
#define ONE_MOVE_VAL (TRACK_MAX_VAL/16)
	...
	HAL_CmdBufIn(VERTEX2F((x+TOP_FORM_W)*EVE_PIXEL_UNIT,(y+4*TOP_FORM_H)*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(END());
...
FTVOID touch_prbs (FTU8 Tag)
{
	...
	if (chanx < CHANX_MAX-1) {
		HAL_CmdBufIn(VERTEX2F(x*EVE_PIXEL_UNIT,y*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((x+CHANX_PARA_W)*EVE_PIXEL_UNIT,(y+3+4*CHANX_PARA_FH)*EVE_PIXEL_UNIT));
		CoCmd_TRACK(x,y,1,4*CHANX_PARA_FH,eT_M_MAX);
	} else {
		HAL_CmdBufIn(VERTEX2F(x*EVE_PIXEL_UNIT,(y-3-4*CHANX_PARA_FH)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((x+CHANX_PARA_W)*EVE_PIXEL_UNIT,y*EVE_PIXEL_UNIT));
...
FTVOID touch_all_polarity (FTU8 *pTag)
{
	...
	HAL_CmdBufIn(BEGIN(RECTS));
	HAL_CmdBufIn(CHANX_COL_PR);
	HAL_CmdBufIn(VERTEX2F(x*EVE_PIXEL_UNIT,(y-2*CHANX_PARA_FH-3)*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F((x+CHANX_PARA_W)*EVE_PIXEL_UNIT,y*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(END());
...
FTU8 touch_keyboard (FTU16 x, FTU16 y, FTU8 *pTag)
{
	...
	/* keyboard background */
	HAL_CmdBufIn(BEGIN(RECTS));
	HAL_CmdBufIn(LINE_WIDTH(CHANX_LINE_W * EVE_PIXEL_UNIT));
	HAL_CmdBufIn(CHANX_COL_PR);
	HAL_CmdBufIn(VERTEX2F(x*EVE_PIXEL_UNIT,(y-CHANX_KEY_H)*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F((x+CHANX_KEY_W)*EVE_PIXEL_UNIT,(y+5*CHANX_KEY_H+5)*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(END());

	/* keyboard keys */
	CHANX_KEY_COL;
	HAL_CmdBufIn(COLOR_RGB(0,0,0));
	HAL_CmdBufIn(LINE_WIDTH(1 * EVE_PIXEL_UNIT));
	if (TOUCHED) {
...
{
#define L1_X (EVE_LCD_WIDTH/2-(LOGO1_W+LOGO2_W+LOGO_GAP)/2)
#define L1_Y (EVE_LCD_HIGH/2-LOGO1_H/2)
#define L2_X (EVE_LCD_WIDTH/2+(LOGO1_W+LOGO2_W+LOGO_GAP)/2-LOGO2_W)
#define L2_Y (L1_Y+5)
#define LOGO_SPEED 10

	static FTU32 x1 = L1_X, y1 = 0, x2 = L2_X, y2 = EVE_LCD_HIGH, ret = 1;
	...
	HAL_CmdBufIn(CELL(0));
	HAL_CmdBufIn(VERTEX2F(x1*EVE_PIXEL_UNIT,
	                      y1*EVE_PIXEL_UNIT));
	
	HAL_CmdBufIn(BITMAP_HANDLE(LOGO_HANDLE+1));
	HAL_CmdBufIn(CELL(0));
	HAL_CmdBufIn(VERTEX2F(x2*EVE_PIXEL_UNIT,
	                      y2*EVE_PIXEL_UNIT));
...
FTVOID chanX_body_form (FTU8 chan_x)
{
	...
	if (chan_x == 4) {
		...
		HAL_CmdBufIn(BEGIN(RECTS));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+2*CHANA_PARA_GAP+CHANX_PARA_W+CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+2*CHANA_PARA_GAP+2*CHANX_PARA_W-CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U+CHANX_PARA_FH)*EVE_PIXEL_UNIT));

		HAL_CmdBufIn(TAG(eT_M_CA_POLY));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+4*CHANA_PARA_GAP+3*CHANX_PARA_W+CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+4*CHANA_PARA_GAP+4*CHANX_PARA_W-CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U+CHANX_PARA_FH)*EVE_PIXEL_UNIT));
		
		HAL_CmdBufIn(TAG(eT_M_CA_GT));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+6*CHANA_PARA_GAP+5*CHANX_PARA_W+CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+6*CHANA_PARA_GAP+6*CHANX_PARA_W-CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U+CHANX_PARA_FH)*EVE_PIXEL_UNIT));

		HAL_CmdBufIn(TAG(eT_M_CA_AMP));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+8*CHANA_PARA_GAP+7*CHANX_PARA_W+CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+8*CHANA_PARA_GAP+8*CHANX_PARA_W-CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U+CHANX_PARA_FH)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(END());
		HAL_CmdBufIn(TAG_MASK(0));
		...
		HAL_CmdBufIn(BEGIN(RECTS));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_PARA_L+CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_PARA_L+CHANX_PARA_W-CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U+CHANX_PARA_FH)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(TAG_MASK(1));
		...
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_PARA_L+CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U+CHANX_PARA_H+2)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_PARA_L+CHANX_PARA_W-CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U+CHANX_PARA_H+CHANX_PARA_FH)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(TAG_MASK(0));

		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_PARA_L+CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U+2*CHANX_PARA_H+2)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_PARA_L+CHANX_PARA_W-CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U+2*CHANX_PARA_H+CHANX_PARA_FH)*EVE_PIXEL_UNIT));

		/* right */
		/*
		HAL_CmdBufIn(TAG_MASK(1));
		HAL_CmdBufIn(TAG(eT_M_C1_POLY+chan_x*eCHANX_OFFSET));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_PARA_R+CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_PARA_R+CHANX_PARA_W-CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U+CHANX_PARA_FH)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(TAG_MASK(0));
		*/
		HAL_CmdBufIn(TAG_MASK(1));
...
FTVOID chanX_body_line (FTU8 chan_x)
{
	...
	HAL_CmdBufIn(LINE_WIDTH(1 * EVE_PIXEL_UNIT));
	if (chan_x == 4) {
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+2)*EVE_PIXEL_UNIT,
				(chanX_list[chan_x].Y+CHANX_BAR_H+CHANA_BUTN_LN)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANA_W-2)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANA_BUTN_LN)*EVE_PIXEL_UNIT));
	} else {
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+2)*EVE_PIXEL_UNIT,
				(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_BUTN_LN)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_W-2)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_BUTN_LN)*EVE_PIXEL_UNIT));
	}
	HAL_CmdBufIn(END());
}
...
FTVOID chanX_body (FTU8 chan_x, FTU8 chan_tag)
{
	...
	HAL_CmdBufIn(LINE_WIDTH(CHANX_LINE_W * EVE_PIXEL_UNIT));
	if (chan_x == 4) {
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANA_W-CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANA_BODY_H)*EVE_PIXEL_UNIT));
	} else {
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_W-CHANX_LINE_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_BODY_H)*EVE_PIXEL_UNIT));
	}
	HAL_CmdBufIn(END());
...
FTVOID chanX_bar (FTU8 chan_x, FTU8 chan_tag, FTU8 led_red)
{
	...
	HAL_CmdBufIn(LINE_WIDTH(1 * EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F(chanX_list[chan_x].X*EVE_PIXEL_UNIT,
	                      chanX_list[chan_x].Y*EVE_PIXEL_UNIT));
	if (chan_x == 4) {
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANA_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H)*EVE_PIXEL_UNIT));
	} else {
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_W)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H)*EVE_PIXEL_UNIT));
	}
	HAL_CmdBufIn(END());
	...
	if (chan_x == 4) {
		...
		HAL_CmdBufIn(POINT_SIZE(CHANX_LED_RA*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_LED)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H/2)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(END());
	}
...
FTVOID chanX_triangle (FTU8 chan_x)
{
	...
	HAL_CmdBufIn(CELL(0));
	if (chan_x == 4) {
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+2*CHANA_PARA_GAP+2*CHANX_PARA_W-CHANX_LINE_W-TRI_W-TRI_GAP)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U+CHANX_PARA_FH/2)*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+4*CHANA_PARA_GAP+4*CHANX_PARA_W-CHANX_LINE_W-TRI_W-TRI_GAP)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U+CHANX_PARA_FH/2)*EVE_PIXEL_UNIT));
	} else {
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_PARA_L+CHANX_PARA_W-CHANX_LINE_W-TRI_W-TRI_GAP)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U+CHANX_PARA_FH/2)*EVE_PIXEL_UNIT));
		/*
		HAL_CmdBufIn(VERTEX2F((chanX_list[chan_x].X+CHANX_PARA_R+CHANX_PARA_W-CHANX_LINE_W-TRI_W-TRI_GAP)*EVE_PIXEL_UNIT,
					(chanX_list[chan_x].Y+CHANX_BAR_H+CHANX_PARA_U+CHANX_PARA_FH/2)*EVE_PIXEL_UNIT));
		*/
	}
	HAL_CmdBufIn(END());
...
FTVOID top_ui (FTVOID)
{
	...
	HAL_CmdBufIn(VERTEX2F(TOP_FORM_X*EVE_PIXEL_UNIT,TOP_FORM_Y*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F((TOP_FORM_X+TOP_FORM_W)*EVE_PIXEL_UNIT,(TOP_FORM_Y+TOP_FORM_H)*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(TAG_MASK(0));
	HAL_CmdBufIn(END());
	HAL_CmdBufIn(COLOR_RGB(255, 255, 255));
	...
	HAL_CmdBufIn(BITMAP_HANDLE(LOGO_HANDLE));
	HAL_CmdBufIn(CELL(0));
	HAL_CmdBufIn(VERTEX2F(LOGO1_X*EVE_PIXEL_UNIT,
	                      LOGO1_Y*EVE_PIXEL_UNIT));

	HAL_CmdBufIn(BITMAP_HANDLE(LOGO_HANDLE+1));
	HAL_CmdBufIn(CELL(0));
	HAL_CmdBufIn(VERTEX2F(LOGO2_X*EVE_PIXEL_UNIT,
	                      LOGO2_Y*EVE_PIXEL_UNIT));

	/* Top, black triangle */
	HAL_CmdBufIn(BITMAP_HANDLE(LOGO_HANDLE+2));
	HAL_CmdBufIn(CELL(0));
	HAL_CmdBufIn(VERTEX2F((TOP_FORM_X+TOP_FORM_W-TRI_W-TRI_GAP)*EVE_PIXEL_UNIT,
	                      (TOP_FORM_Y+TOP_FORM_H/2)*EVE_PIXEL_UNIT));
