...
STATIC FTVOID init_bubble (FTVOID)
{
	...
	for (i = 0; i < BUBBLE_NUM; i++) {
		Bubble[i].x = FTRANDOM(EVE_PIXEL_UNIT*EVE_LCD_WIDTH);
		Bubble[i].y = FTRANDOM(EVE_PIXEL_UNIT*EVE_LCD_HIGH);
	}
}
...
STATIC FTVOID pos_bubble (bubble_st *p)
{
	...
	for (k = 0; k < BUBBLE_NUM; ++k) {
		...
		p[k].x += p[k].xOffset;
		if (p[k].x < EVE_PIXEL_UNIT*EVE_LCD_WIDTH/3) {
			/* set the border of effect */
			p[k].x = EVE_PIXEL_UNIT*EVE_LCD_WIDTH/3 + BUBLE_OUT_SIZE(BUBLE_HI_2);
		}
		p[k].y += p[k].yOffset;
	}
...
#endif
#define X_E1_POS (EVE_LCD_WIDTH/3)
#define Y_V_POS  (EVE_LCD_HIGH/2)
#define X_E2_POS (EVE_LCD_WIDTH/3*2)
#define POS_SEED   (15)
...
STATIC FTU32 ui_standby_point_show (FTU32 X)
{
	...
	static FTU32 size = 5;
#define KEY_LEN            (EVE_LCD_HIGH/4)
#define KEY_LINE1_X1        (EVE_LCD_WIDTH/4*EVE_PIXEL_UNIT)
#define KEY_LINE1_Y1        ((EVE_LCD_HIGH/2-KEY_LEN/2)*EVE_PIXEL_UNIT)
#define KEY_LINE1_Y2        (EVE_LCD_HIGH/2*EVE_PIXEL_UNIT)
#define KEY_POINT_X1       KEY_LINE1_X1
#define KEY_POINT_Y1       (KEY_LINE1_Y1+KEY_LEN/8*7*EVE_PIXEL_UNIT)
#define KEY_POINT_SIZE_OUT (KEY_LEN/8*EVE_PIXEL_UNIT)
#define KEY_POINT_SIZE_IN  (KEY_LEN/16*EVE_PIXEL_UNIT)
...
#define KEY_LINE2_Y2        (KEY_LINE1_Y2+KEY_LEN/16*EVE_PIXEL_UNIT)

#define LOCK_X1             ((EVE_LCD_WIDTH/4*3-KEY_LEN/4)*EVE_PIXEL_UNIT)
#define LOCK_Y1             (EVE_LCD_HIGH/2*EVE_PIXEL_UNIT)
#define LOCK_X2             (LOCK_X1+KEY_LEN/2*EVE_PIXEL_UNIT)
#define LOCK_Y2             (LOCK_Y1+KEY_LEN/2*EVE_PIXEL_UNIT)
#define LOCK_POINT_X        (LOCK_X1+KEY_LEN/4*EVE_PIXEL_UNIT)
#define LOCK_POINT_Y        (LOCK_Y1+KEY_LEN/4*EVE_PIXEL_UNIT)
#define LOCK_LINE1_X        (EVE_LCD_WIDTH/4*3*EVE_PIXEL_UNIT)
#define LOCK_LINE1_Y        (LOCK_Y1-KEY_LEN/8*EVE_PIXEL_UNIT)
#define LOCK_LINE1_W        (KEY_LEN/8*EVE_PIXEL_UNIT)

	/* draw key */
	if (X < KEY_LINE1_X1/EVE_PIXEL_UNIT) {
		/* touch area */
		...
		HAL_CmdBufIn(POINT_SIZE(KEY_LEN/2*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F(EVE_LCD_WIDTH/4*EVE_PIXEL_UNIT, EVE_LCD_HIGH/2*EVE_PIXEL_UNIT));
		...
		HAL_CmdBufIn(VERTEX2F(KEY_LINE2_X1, (KEY_LINE2_Y1+KEY_LEN/8*EVE_PIXEL_UNIT)));
		...
		HAL_CmdBufIn(BEGIN(FTPOINTS));
		HAL_CmdBufIn(POINT_SIZE(KEY_LEN/2*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(VERTEX2F(X*EVE_PIXEL_UNIT, EVE_LCD_HIGH/2*EVE_PIXEL_UNIT));
		HAL_CmdBufIn(TAG_MASK(0));
		
		HAL_CmdBufIn(COLOR_PNT_STANDBY);
		HAL_CmdBufIn(BEGIN(LINES));
		HAL_CmdBufIn(LINE_WIDTH(KEY_LINE_W));
		HAL_CmdBufIn(VERTEX2F((X-KEY_LEN/2)*EVE_PIXEL_UNIT, KEY_LINE1_Y2));
		HAL_CmdBufIn(VERTEX2F((X+KEY_LEN/2)*EVE_PIXEL_UNIT, KEY_LINE1_Y2));
		HAL_CmdBufIn(LINE_WIDTH(KEY_LINE_W*2));
		HAL_CmdBufIn(VERTEX2F((X+KEY_LEN/8)*EVE_PIXEL_UNIT, KEY_LINE2_Y2));
		HAL_CmdBufIn(VERTEX2F((X+KEY_LEN/4)*EVE_PIXEL_UNIT, KEY_LINE2_Y2));
	...
	HAL_CmdBufIn(VERTEX2F(LOCK_LINE1_X, (LOCK_LINE1_Y+KEY_LEN/8*EVE_PIXEL_UNIT)));

	/* draw lock: body in */
	HAL_CmdBufIn(COLOR_PNT_BAKGRD);
	HAL_CmdBufIn(BEGIN(FTPOINTS));
	if (X >= (LOCK_X1/EVE_PIXEL_UNIT)) {
		HAL_CmdBufIn(POINT_SIZE(KEY_POINT_SIZE_OUT+size*EVE_PIXEL_UNIT));
		size += 5;
		if (size >= KEY_LEN/2) {
			alpha = 0;
			flag = 0;
...
FTU8 menu_a (FTVOID)
{
#define G_X (EVE_LCD_WIDTH/3*2)
#define G_Y (EVE_LCD_HIGH/2)
#define G_R (EVE_LCD_HIGH/3)
#define G_L (EVE_LCD_HIGH-EVE_LCD_HIGH/16)
#define G_F (31)
#define G_MA 8
#define G_MI 3
#define G_RG 255
	...
	HAL_CmdBufIn(VERTEX2F(EVE_LCD_WIDTH/3*EVE_PIXEL_UNIT,0));
	HAL_CmdBufIn(VERTEX2F(EVE_LCD_WIDTH*EVE_PIXEL_UNIT,EVE_LCD_HIGH*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(END());
	...
	HAL_CmdBufIn(BEGIN(LINES));
	HAL_CmdBufIn(VERTEX2F((G_X-G_R/2)*EVE_PIXEL_UNIT,G_L*EVE_PIXEL_UNIT));
	HAL_CmdBufIn(VERTEX2F((G_X+G_R/2)*EVE_PIXEL_UNIT,G_L*EVE_PIXEL_UNIT));
...
FTU8 menu_b (FTVOID)
{
#define PRG_X (EVE_LCD_WIDTH/3+EVE_LCD_WIDTH/12)
#define PRG_Y (EVE_LCD_HIGH/2)
#define PRG_W (EVE_LCD_WIDTH/3*2-EVE_LCD_WIDTH/6)
#define PRG_R (255)
#define PRG_F (31)
#define PRG_H (20)
	static FTU8 value = 0, flag = 0;
	...
	CoCmd_NUMBER((EVE_LCD_WIDTH/3*2),(EVE_LCD_HIGH/3),PRG_F,OPT_CENTER,value);
...
FTU8 menu_c (FTVOID)
{
#define SLD_X1 (EVE_LCD_WIDTH/3+EVE_LCD_WIDTH/6)
#define SLD_X2 (EVE_LCD_WIDTH/3*2)
#define SLD_X3 (EVE_LCD_WIDTH/3*2+EVE_LCD_WIDTH/6)
#define SLD_Y (EVE_LCD_HIGH/6)
#define SLD_W (EVE_LCD_WIDTH/64)
#define SLD_H (EVE_LCD_HIGH/3*2)
#define SLD_RG (255)
...
STATIC FTU8 ui_main_title_show (FTU8 hide, FTU8 title_index)
{
	...
	for (i = 0,tag = TAG_MENU; i < MENU_NUM;i++,tag++) {
		...
		HAL_CmdBufIn(COLOR_MENU_TITLE);
		CoCmd_BUTTON(0,(EVE_LCD_HIGH/MENU_NUM)*i,EVE_LCD_WIDTH/3, \
				     EVE_LCD_HIGH/MENU_NUM,MENU_TITL_FONT,OPT_FLAT,&pMenuTitle[i][0]);
	}
	HAL_CmdBufIn(TAG_MASK(0));
...
#define CLK_M   (40)
	...
	CoCmd_CLOCK(EVE_LCD_WIDTH-CLK_R,CLK_R,CLK_R,OPT_NOBACK,0,0,(CLK_H*CLK_H2S+CLK_M*CLK_M2S+s++),0);
